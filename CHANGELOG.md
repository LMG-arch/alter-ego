...
- **设置页可以直接加 / 改模型端点**。`llm.providers` 原本在页面上是一句灰字
  （「这一项是一整段表，请直接改配置文件」），现在有真正的编辑器：控件类型从**文件里
  现有的那一行**推断（`infer_setting_from_value`——`bool` 必须先于 `int` 判断，
  否则开关会画成数字框），字段名补全来自用户自己的配置，猜不出来就灰字显示
  「这一行请在文件里改」而**不回落成字符串**。新增 `GET` / `POST
  /api/settings/providers`；一次改多行、**一次落盘**（`save_settings`），避免留下
  「`base_url` 换过了、`model` 还没换」的半个端点。名字像密钥的行（`api_key`）
  只回「设了没有」且拒绝写回，`api_key_env` 是例外——它的值是变量名，写错了不会报错，
  只会安静地拿着空密钥去请求。**不提供删除**：删掉一个还被 `[llm.routing]` 指着的端点
  会让下次启动直接失败，而文本补丁回滚不了两步（只有一份 `.bak`）
...
**主体四块 · 第二批之一：插件命令组（`alterego plugins`）**

计划见 `docs/plans/2026-09-16-main-body.md` § 4 批次 B。「主体」四块里第二块
（插件加载）的第一半，也是第一次能**从命令行**看清「插件到底装成什么样」：

```bash
alterego plugins list                 # 发现到的、哪些会加载、清单坏在哪个目录
alterego plugins doctor               # 真的装一遍，报加载结果与健康状态
alterego plugins info capability.example   # 一个插件的清单、配置现状、会不会被加载
alterego plugins reload capability.example # 卸掉再装一次
alterego plugins reset <PLUGIN_ID>    # 解除熔断
```

- **`cli_plugins.py`（第八个组装根）**。它只调 `PluginManager` 已有的方法，
  没有新增一条插件 API；`PluginManager` 也一行没改——这正是「插件加载已经写好了，
  缺的只是把它接到命令行上」的证据
- **`list` 与 `info` 刻意不导入插件代码**。它们最常被用来回答的正是
  「这个插件为什么没生效」，而答案如果是「它的代码根本导入不了」，
  那么一个必须先导入它才能跑的命令什么也证明不了——它会和插件一起坏掉。
  `tests/test_cli_plugins.py::test_listing_does_not_import_the_plugins`
  用一个 `import` 就抛异常的插件钉住这条
- **`list` 的输出是分层的，而且每一层都对着一个真实的追问**：
  表（发现 3 个 / 启用 0 个）→「一个都没启用」时给出**能直接抄进配置文件的那一行**
  （不是「请去配置」）→「清单读不出来的目录」单独一节
  （清单坏了的插件不会出现在表里，只打一张表会让人以为那个目录被忽略了）→
  「一个插件都没发现」时列出**它找过哪些目录**
- **`doctor` 的退出码是三个而不是两个**。依赖成环 / 缺依赖 → 3
  （`EXIT_DEPENDENCY_ERROR`，与 `02-plugin-api.md` § 8.2 一致），
  有插件没装上或不健康 → 1，全好 → 0。脚本要能把「配置写错了」和
  「插件之间接不起来」分开。插件**导入就炸**时它照样跑到底并报告，
  而不是自己也炸掉
- **`_manager()` 显式按 `config.core.random_seed` 播种 rng**。不播种的话每次启动
  抖动都不一样，而 `doctor` 的输出是要能对得上的（P6）
- **`reset` 在一次性进程里几乎必然无事可做**（熔断状态活在 `PluginManager` 实例的
  内存里，而这条命令每次都是一个新进程），所以它对「没有处于熔断状态的插件」这句话
  是**照实说**，并把「为什么」也打出来。写一个假装做成了什么的空操作，比不写更坏
- **红线 11（`sqlite` 实现只被组装根与存储层引用）的豁免名单没有加 `cli_plugins.py`**：
  它一行存储代码都不需要，加进去只会让这条红线少拦一个文件。
  注释里把这个判断写下来了——**豁免名单越短，检查越强**
- 测试新增 46 个（`tests/test_cli_plugins.py`），模块覆盖率 **99.5%**。
  唯一没覆盖到的是 `_plugins_config()` 的真实实现（测试按约定换掉它）
  和一处「重载返回 False 且插件已被移出」的防御分支
- 写这一批时发现的**测试自身的两处坑**，都是「合成数据缺了必填项」：
...
**主体四块 · 第三批之补：设置页的 12 个分组可以收起来了（已实现）**

- 设置页是 **96 项 / 12 组**，一屏只放得下两组。每个分组改用**原生
  `<details>` / `<summary>`** 折叠：浏览器自带展开收起、Tab + 回车就能操作、
  `::marker` 那个三角本身就是「这里能点」的线索。`app.js` 为此只多写了一处
  `toggle` 监听，其余全交给浏览器——不引框架、不加 `role` / `tabindex` / `aria-expanded`
- **默认全部展开。** 收起是给「已经知道自己要找哪一组」的人用的姿势，
  把 96 项藏在 12 次点击后面，最吃亏的是那种「只想抄一下某个键名」的人
- **收起来的分组会被记住**（`state.collapsed`）。`render()` 会把整个面板重画，
  刷新、切页签回来都算一次重画；不记的话，用户刚收起的组会在他什么都没做的时候
  自己弹开，看起来就是界面坏了
- `tests/test_web_static.py` 从 2 个增加到 4 个，守着两条**接口层测试结构上看不见**的
  不变式：分组必须是带 `<summary>` 的 `<details>`（换回 `<h2>` 就再也没有「收起」这个
