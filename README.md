...
它的日子摊开是一间 Obsidian 库：日程、想法、读到的东西、记得的事、见过的人、
它自己学的专业各占一个目录，每天一页日程写着「打算做什么 / 实际做了什么 / 心里想着什么」。
...
它不是只会聊。人设里写着一句职业，它就自己去把那个方向一格一格学下去——
学到的每一格写成一篇笔记，和「见过的人」「记得的事」**在同一间库里**（`60-专业/`），
以后聊到那个方向时会自己翻出来用。

> **「会自己翻出来用」到哪一步了**：打分与召回这套机制是**做完了的**，
> `alterego study recall "<一句话>"` 能让你亲眼看到它会翻哪几篇、靠哪几个词命中。
> 但**「每轮对话自动塞进去」还没接线**——那个调用点在推演/对话循环里，
> 而那条循环本身还没落地（`alterego chat` 仍是规划）。两条都做完，
> 这件事才算闭环；在那之前，它是一个**可以当场验证的机制**，
> 而不是一句承诺。已备好但未接线的还有 `prompts/emotion_update.md`，
> 理由一样：不假装接线了。

一个方向拆成**五格**：`是什么` / `怎么做` / `容易踩的坑` / `和什么容易混` / `我还不服的`。
前两格是「能讲清楚」，后三格是「知道边界」——最后那一格最关键：
一个只会复述的角色和一个会挑刺的角色读起来是两种人，而**挑刺是可以被安排出来的**。

```bash
alterego study status              # 学到哪了、库里有什么（不花钱，也不要密钥）
alterego study plan --rounds 5     # 接下来五格是哪几个题面（不花钱）
alterego study recall "降维怎么做" # 给一句话，看它会翻出哪几篇（带命中的词）
alterego study next                # 真的学一格（这一步花钱）
alterego study next --dry-run      # 先看看会学什么，不写文件
```

``alterego study next`` 默认一次只学**一格**（``[study] rounds`` 是格数，不是轮数）：
一次学完整个方向，就把「我一个星期前还不懂这个」这件事抹掉了，
而那正是这个功能唯一能证明自己的东西。**模型答得不完整就不记进度，下次还会来学这一格。**

认不出你该学什么时它会直接说（比如人设里写着「研究生」但没写哪个专业），
而不是随便挑一个方向先去学——**瞎猜一门学问的后果是它写出一堆像模像样的空话，
而没人会想到要去检查**。想指定就填一句 ``[study] field``。
