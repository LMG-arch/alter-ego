...
- 数据集的三类源表现今**都有写入者**（`message` / `conversation` / `tick_log` / `activity_log`），
  但空结果照样会被如实报成「0 条」，并区分三种原因（上游没落地 / 时间范围小了 /
  取到行但拼不出样本）——不假装，也不先生成假数据
...
- **`upstream_ready` 是手写的，会过期。** 它曾经钉在 `tick_log` / `activity_log` 上
  （理由「`sim/` 尚未实现」），推演引擎落地后没人回改，于是 CLI 把
  「你时间范围开小了」统报成「上游还没落地」——一个只有真跑一次导出才会看见的错。
