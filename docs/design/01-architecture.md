...
与 `storage/` 自己引用。例外写在脚本的注释里，而不是靠「这一条先关掉」——
...
**与上面这份草图的差别**（2026-09-15 记）：真实实现是
`src/alterego/cli.py` + `cli_io.py` + `cli_db.py`，**没有** `cli/commands/` 目录。
「一个命令一个模块」在命令总数还是个位数时只带来导入开销与目录层次，
而「单文件 ≤ 900 行」这条上限用「超了再拆」处理更划算——
三次拆分的边界（参数树 / 输出助手 / 某一组命令）都是撞到上限时才看清的，
