...
# 红线 7：channels/ 不得读进程时区取时间（要用 core.timezone）
! grep -rEn "datetime\.now\(|date\.today\(" src/alterego/channels/
```

上面是示意，实际实现是 `scripts/check_architecture.sh` 里的 **7 组 24 项**检查。对应关系：

| 脚本分组 | 覆盖的红线 | 项数 |
| --- | --- | --- |
| 第 1 组 内核无知 | 红线 1 | 4 |
| 第 2 组 领域层纯净 | 红线 2 | 4 |
| 第 3 组 推演层抽象 | 红线 3 / 4 + 可复现性 + 组装根例外 | 6 |
| 第 4 组 分层不越级 | —— | 2 |
| 第 5 组 通用卫生 | **红线 6 / 7** + 文件行数上限 | 5 |
| 第 6 组 插件自包含 | —— | 1 |
| 第 7 组 LLM 调用必经 `ctx.llm()` | **红线 5** | 2 |

第 3 组最后一项管的是**组装根例外**：具体存储实现只准被 `cli.py` / `cli_db.py` /
`cli_memory.py` / `cli_vault.py` 与 `storage/` 自己引用。例外写在脚本的注释里，
而不是靠「这一条先关掉」——
**一条被关掉的检查和一条不存在的检查，在输出上长得一模一样。**
...
**红线 7 管的是「它的几点」。** `channels/` 里算「现在」必须走 `deps.now` / `deps.today`，
它们读 `core.timezone`；写成 `datetime.now().astimezone()` 或 `date.today()` 读的是
**跑这个进程的那台机器**的时区。这两句话在开发机上恰好等价（本机也装着东八区），
在 UTC 的容器里差 8 小时——而 `/api/budget` 拿这个时刻取**日期**，差 8 小时就是
差一天：页面会显示一个引擎从没写过的那天，症状是「它明明发过消息，额度却显示 0」。
`cli.py::_today`（注释里明写了 `DTZ011`）与 `kernel/scheduler.py` 早就按配置时区取
...
| 红线 7（`channels/` 读进程时区） | 换一台服务器的那天——本机装的时区和配置里的恰好一样，所以本地永远是对的 |
...
**与上面这份草图的差别**（2026-09-16 记）：真实实现是
`src/alterego/cli.py` + `cli_io.py` + `cli_db.py` + `cli_memory.py` + `cli_vault.py`，
**没有** `cli/commands/` 目录。
「一个命令一个模块」在命令总数还是个位数时只带来导入开销与目录层次，
而「单文件 ≤ 900 行」这条上限用「超了再拆」处理更划算——
四次拆分的边界（参数树 / 输出助手 / 某一组命令）都是撞到上限时才看清的，
而它们预先并不明显。`cli.py` 一度 937 行，拆出 `cli_db.py` 后两边都回到六百行以下。

后来两个命令组（`memory` / `vault`）是**直接按这个先例新建文件**的，没有再让
`cli.py` 涨回上限——第一次撞墙换来的是一条可复用的边界。它们还共用同一个形状：
每个命令组自己开库、自己拿供应商、自己关掉，`cli.py` 只负责挂参数树与统一
退出码（`0` 成功 / `2` 用户错误 / `4` 迁移失败）。

`export` / `import` / `stats` / `persona` 等命令尚未实现；
命令树的全量清单见 [`05-channels.md`](05-channels.md) § 8.1。
...
| 码 | 含义 | 典型场景 |
| --- | --- | --- |
| 0 | 正常 | 命令成功；`--help`；只敲到某一层组名 |
| 1 | 通用错误 | 未预料的异常（这应当被修掉，而不是被用户遇到） |
| 2 | 配置非法 | 配置文件读不出来、字段越界、库版本不兼容、备份文件不合法 |
| 3 | 插件依赖环 | 清单里出现环 |
| 4 | 数据库迁移失败 | 迁移脚本本身出错、库比程序新 |

**异常类不携带退出码。** 「发生了什么」是异常的事，「这个进程该以几退出」
是调用方的事——`main()` 里按异常类型分派。让每个异常类自己声明 `exit_code`
看起来很省事，代价是这张表散落在十来个类里，而没有任何一处能一眼看全。

`4` 与 `2` 分开是有用的：迁移失败时人是**该去看备份**的，
而配置写错时该去看配置文件。同一个退出码下这两件事分不开。
...
| 2026-09-16 | v0.1.2 | § 1 新增红线 7（`channels/` 不读进程时区）与第 5 组的第 24 项检查；分组表里的项数 23 → 24。理由：这一条在开发机上与配置默认值恰好等价，只有在 UTC 的机器上才差 8 小时——**只能靠人记住的规则不是规则** | LMG-arch |
