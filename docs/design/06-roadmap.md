...
| 项 | 验收标准 |
| --- | --- |
| Web | **13 个页面**可用（总览/对话/动态/时间线/关系网/内心/记忆/相册/统计/日志/信息源/设置/后台）。⚠️ v0.1.2 已交付 **10 个**；关系网 / 相册 / 日志 缺的不是页面而是路由（仓储没有对应读方法），见 `05-channels.md` § 3.3 的「实现状态」 |
| SSE | 新消息实时推送；断线自动重连；心跳保持 |
| 认证 | token 认证生效；`auth = "none"` + `host = 0.0.0.0` 被拒绝启动（v0.1.2 已生效） |
