...
> § 7 的页面结构已部分落地：设置页（`app.js` 的 `views.settings`）能读能写，
> 写回只做「标记待重启」；§ 7.1 的 TOML 编辑器与 § 7.2 的成本矩阵还是规划。
...
| 开放容器 | `llm.providers.<名字>.<字段>` | 按值渲染，无 `effect`；猜不出来就不给编辑 |

**第三层必须存在**：用户会在 `alterego.toml` 里手写自己的插件配置。如果设置页
遇到未知键就报错或静默忽略，用户会觉得「我的配置丢了」。

### 3.1 第四层：从**值**推断（`infer_setting_from_value`）

前两层从声明来，第三层从**类型注解**来，第四层从**值本身**来。它存在的理由是
开放容器：`llm.providers` 声明成 `Mapping[str, Any]`，里面有哪些键由那个 provider
的插件解释（P4），内核既不知道、也不该替它编一份键名清单。

**那份清单一定会漂移。** 如果内核声明「provider 该有 `base_url` / `model` /
`api_key_env`」，插件加一个 `extra_headers`、或者改名一个键，页面就开始教用户
写一个插件根本不读的键——而且写错了不报错，只是读不到。所以：

| 问题 | 依据 |
| --- | --- |
| 这一行是什么类型 | **文件里现有的那个值**（`3` → 数字框，`true` → 开关） |
| 补全列表里有哪些字段名 | 用户的配置里出现过的名字（`known_fields`） |
| 猜不出来怎么办 | 灰字显示「这一行请在文件里改」，**不回落到字符串** |

**「不回落」是硬性要求。** 把一个嵌套的表当成字符串渲染，用户改完一保存，写进
文件的是一个字符串字面量；下一次启动时 provider 报的错和用户刚才做的那件事看起来
毫无关系。

两条例外，都是「名字说明它不是值」：

- **名字像密钥的行**（命中 `_SECRET_KEY_PATTERN`）只回 `{"set": true}`，并且不可编辑。
  provider 插件允许把 `api_key` 直接写在文件里，而**回显它就是把密钥送进浏览器**。
- **以 `_env` 结尾的键不算密钥**。`api_key_env` 的值是一个**变量名**，而页面应该
  让人看见它、也应该让人改它——那个名字写错了，启动时不会报错，只会安静地拿着
  一个空密钥去请求。
...
| 模型 | **端点（可加、可改）**、强/中/弱别名、**每个用途的绑定**（10 个下拉） |
...
上面那张表里的十几个分组，实机上是 **96 项 / 12 组**。一屏放得下两组，
想跳到「存储」要么滚很久、要么先切页签。所以每个分组用**原生
`<details>` / `<summary>`** 折叠。

选原生元素而不是「自己画一个箭头加 click 事件」，是因为三件事一起省掉了：

| 原生给的东西 | 自己写要付出的 |
| --- | --- |
| 展开 / 收起的行为 | 一个状态、一个 `hidden`、一次重画 |
| 键盘可达（Tab 聚焦 + 回车） | `tabindex` / `role` / `aria-expanded` 一样不少 |
| `::marker` 那个三角 | 一条「这里能点」的视觉线索，箭头图标或伪元素 |

`app.js` 在这件事上只多写了一处 `toggle` 监听，其余全交给浏览器。

⚠️ **`::marker` 不能删。** 它是「这一行能点」唯一的视觉线索；
把它去掉、又没补上自己的箭头，收起之后那一行看起来就是一段普通文字，
没人会想到去点它。

**默认全部展开。** 收起是为「已经知道自己要找哪一组」的人准备的姿势，
不是所有人的默认姿势——把 96 项藏在 12 次点击后面，
最吃亏的是那种「只想抄一下某个键名」的人。要的是「能收起」，
不是「默认收起」。

**收起来的分组会被记住。** `render()` 会把整个面板重画一遍，
刷新页面、切到别的页签再回来，都算一次重画；不记的话，用户刚收起的
分组会在他什么都没做的时候自己弹开，看起来就是界面坏了。

`tests/test_web_static.py` 守着两条：

1. 分组必须是**带 `<summary>` 的 `<details>`**——换回 `<h2>` 的那一刻，
   这一页就再也没有「收起」这个功能了，而且没有任何报错，只是点不动；
2. 展开与否必须**读 `state.collapsed`**，并且把每次开合**记回去**——
   写死 `open` 就是「刷新就弹开」，只读不写就是「那个集合永远是空的」。

> 和 [`05-channels.md`](05-channels.md) § 7.2 里那个 `hidden` 的坑一样，
> 这两条都是接口层测试**结构上看不见**的：Web 用例全部走 httpx，
> 没有一条会去读 `app.js` 或 CSS。

### 7.5 模型端点在页面上就能加

`llm.providers` 原本在页面上是一句灰字：「这一项是一整段表，请直接改配置文件。」
那个死胡同本身就是事故——这一页有九十多项，用户会把「改不了」读成「还没做」，
最后去改那个没用的地方。所以它现在有真正的编辑器。

通用接口 `POST /api/settings` 的契约是「一个键 → 一个值」，而这里要写的是
「一张表的一部分」，所以另开一对端点：

| 端点 | 作用 |
| --- | --- |
| `GET /api/settings/providers` | 每个端点逐行摊开：值、类型（`kind`）、能不能改（`editable`）、字段名补全列表 |
| `POST /api/settings/providers` | 新增或修改一个端点的几行，**一次写多行、一次落盘** |

字段名与类型一律来自**文件**，不是内核的清单（§ 3.1）。页面据此做三件事：
...
│ model       [ deepseek-chat                      ]           │
│ max_retries [ 3                                  ]  ← int    │
│ extra_headers   这一行是嵌套的表，请在文件里改：{...}          │
│ api_key         已设置，值不经过这个页面                       │
├──────────────────────────────────────────────────────────────┤
│ ▸ 加一个端点                                                  │
└──────────────────────────────────────────────────────────────┘
```

**没有删除。** 删掉一个还被 `[llm.routing]` 指着的端点，下次 `serve` 直接起不来，
而文本补丁删行删段做不到回滚（备份只留一份 `.bak`，连着撤两步就撤不回去了）。
要删就在文件里删——那里顺手改 `[llm.routing]`，本来也是同一件事。

**`added` 要单独说一句。** 响应里的 `added` 是这次新写进去的行名，页面据此提醒
「这一行是新加的」：这一节里的键由 provider 自己解释，**名字写错了不会有任何提示**，
它只是读不到（P3 的反面教材——这里没有机制可加，只能把话说清楚）。

`tests/test_web_routes.py` 的 `# /api/settings/providers` 一节守着这些，其中三条
值得单独点名：

| 测试 | 挡住什么 |
| --- | --- |
| `test_providers_never_hands_out_a_key_written_into_the_file` | 断言**整段响应原文**里找不到密钥，而不只看那一个字段——泄漏常常发生在「顺手多带了一个字段」的地方 |
| `test_a_name_that_cannot_be_a_table_is_400` | `llm.providers.a.b` 在 TOML 里是「a 里面有一张叫 b 的表」，放过去就是「以为加了端点，其实是加了一层嵌套」 |
| `test_a_failed_provider_write_leaves_the_file_alone` | 一次改几行时其中一行不合法，那么一行都不写 |

前端另有一条源码级断言（`tests/test_web_static.py`）：密钥行的判定必须排在
「不是标量」那条分支**前面**。密钥行的 `editable` 也是 `false`，顺序反了的症状是
用户看到「这一行是嵌套的表：`{"set":true}`」——既没说他有个密钥，又把接口的内部
形状显示给他看。
...
| 模型端点里的密钥 | 名字命中 `_SECRET_KEY_PATTERN` 的行只回「设了没有」，且拒绝写回（§ 3.1） |
...
| 2026-09-16 | v0.1.3 | 新增 § 7.4：分组用原生 `<details>` / `<summary>` 收起，默认展开、收起的组记在 `state.collapsed` 里；`test_web_static.py` 补两条守卫（分组必须是带 `<summary>` 的 `<details>`、展开与否必须读并写 `state.collapsed`） | LMG-arch |
