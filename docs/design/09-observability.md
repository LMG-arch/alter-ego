...
> 上级文档：[DESIGN.md](../DESIGN.md) · 版本 v0.1.6
...
「还有词元(token)消耗页面统计」这句话现在有了第一个落点：**统计页上的 token 区块**
（`GET /api/stats/tokens`，见 [05-channels.md § 3.3](05-channels.md)）。
它只做了 § 2.4 里的前四行，其余仍是设计：

| 设计里的 | 现在 | 卡在哪儿 |
| --- | --- | --- |
| 按用途 / 模型 / 天看 token | ✅ `?group=purpose\|model\|day` | — |
| 成功 / 失败分开看 | ✅ `failed` 与 `succeeded` 两个字段 | — |
| 滚动窗口 | ✅ `?days=`（1..365，默认 **1**） | — |
| 金额、预算条、外推预测（§ 2.2 / § 2.5） | ❌ 一个都没有 | 没有价目表，`llm_usage.cost_usd` 恒为 0 |
| `media_usage`（生图）混进同一张表 | ❌ | 生图既无 `prompt_tokens` 也无 `completion_tokens`，混进来会让两边的聚合都变脏（`03-data-model.md`） |
| CLI（§ 2.6） | ❌ | 同一份聚合，先有页面再有命令 |

四条实现口径，写在这里免得下次照着 § 2 的设计稿改回去：

1. **取数口是 `llm_usage`，不是 `v_cost_daily`。** 那个视图是给金额用的，
   它**没有 token 列**（`05-channels.md § 2.3` 的那张表解释过它为什么这么设计：
   视图是取数口不是报表）。分组在 SQL 里做——`llm_usage` 一次调用一行，
   一个月的量早就过了「全取回来在 Python 里数」的价钱。
2. **`?days=` 与 `/api/stats?days=` 是两个窗口。** 后者是「打开页面第一眼」
   那一屏，固定 30 天、可以被 `truncated` 截断；这一页要解释账单，
   所以它数得准、窗口默认只有 1 天。共用一个参数会让「总览」跟着一起变。
3. **失败的那几次照算。** 一次被限流、重试三次才成的调用花了三倍的钱，
   而账本对每一次**尝试**记一行。滤掉失败行会让这一页比账单好看——
   而它的作用恰恰是解释账单。所以 `failed` 计进 `calls`，只是单独给一个字段。
4. **一个金额字段都不给。** § 2.2 要求「未配置单价时显示『—』，而不是 `$0.00`」；
   接口层连 `cost_usd` 都不返回，前端也就没有机会把它画成 `$0.00`。
   这一页要加金额，先要有价目表。

**没有进度条。** § 2.5 的三件事里有一件是「今天已用 / 上限」，而
`llm.budget.max_tokens_per_day` 是按**虚拟日**重置的，这里是滚动窗口——
两者相除会得到一个看着像进度条、其实分子分母不是一回事的东西。
真要显示余量，得先问出「它自己的今天」（`WebDeps.today`），那是另一件事。
...
| 2026-09-16 | v0.1.6 | § 2.7 新增「实现状态」：token 消耗页面落地（`GET /api/stats/tokens`），并记下四条口径——取数口是 `llm_usage` 而不是没有 token 列的 `v_cost_daily`、这个窗口与 `/api/stats` 那个分开、失败的尝试照算但单独给字段、一个金额字段都不给（§ 2.2 的「—」而不是 `$0.00`）；同时写明为什么**这版没有**预算进度条 | LMG-arch |
