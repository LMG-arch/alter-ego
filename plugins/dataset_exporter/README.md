...
最后那一种**今天看不见**：三类源表（`message` / `tick_log` / `activity_log`）
都已有写入者，所以它们不会报「上游还没落地」。留着它是给以后新加、
而源表还没人写的类别用的。
...
- **工具调用轨迹是拼出来的，不是原样记下的。** `activity_log` 没有
  「调了哪个工具、参数是什么、返回什么」这三列：工具名由
  `category/intent` 拼出，参数与产物来自 `detail_json`（能力的产物、失败原因）。
  所以它教的是「什么样的情形该选哪个意图、产物长什么样」，
  而不是一份逐字可复现的调用记录。
- **`reasoning` 依赖 `tick_log` 里有行。** 推演引擎（`sim/engine.py`）
  现在会写 `tick_log`；库里那个时间范围没有对应的行，这一类就是空的，
  输出里会说清是「时间范围里真没记录」还是「取到行但拼不出样本」。
